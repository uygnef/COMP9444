...
def init(env, env_name):
    ...
    action spaces
    """
    global replay_buffer, epsilon, envir_name, dis_min, dis_max, ACTION_DIM_
    replay_buffer = []
...
def get_env_action(action):
    ...
    `init()`
    """
    if envir_name == 'Pendulum-v0':
        action = [dis_min +  (dis_max - dis_min) * action / ACTION_DIM_]
        # print(action)
    return action
...
def get_train_batch(q_values, state_in, replay_buffer):
    ...
    for i in range(0, BATCH_SIZE):
        ...
        if sample_is_done:
            ...
            a = np.argmax(Q_value_batch[i])
            target_val = reward_batch[i] + GAMMA * target_next[i][a]
